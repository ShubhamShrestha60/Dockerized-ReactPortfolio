...
const Skills = () => {
  return (
    <div className='mt-10' id="skills">
        ...
        <h1 className='text-[40px] font-bold'>{AppText.Skills}<span className='text-blue-600'>{AppText.Experties}</span></h1>
        </div>
        <div className='flex flex-col md:flex-row  justify-evenly px-8 md:px-0 mt-8 '>
          <div className='grid grid-cols-4 md:grid-cols-4 gap-8  items-center'>
            {skillsList.map((item, index)=>(
                <div className='w-[60px] transition ease-in-out delay-100 hover:scale-125 bg-blue-50
                 p-3 rounded-full' >
                    <img src={item.icon} 
                    className="w-[60px]"/>
                </div>
